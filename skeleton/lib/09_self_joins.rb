...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
  SELECT
    DISTINCT t.name, a.company, a.num
  FROM
    stops s
  JOIN
    routes a ON s.id = a.stop_id
  JOIN
    routes b ON (a.company = b.company AND a.num = b.num)
  JOIN
    stops t ON t.id = b.stop_id
  WHERE
...
def craiglockhart_to_sighthill
  ...
  FROM
    stops craig_stop
  JOIN
    routes a ON craig_stop.id = a.stop_id
  JOIN
    routes x ON (x.company = a.company AND x.num = a.num)
  JOIN
    stops s ON s.id = x.stop_id
  JOIN
    routes b ON s.id = b.stop_id
  JOIN
    routes y ON (y.company = b.company AND y.num = b.num)
  JOIN
    stops sight_stop ON sight_stop.id = y.stop_id
  WHERE
    craig_stop.name = 'Craiglockhart'
    AND sight_stop.name = 'Sighthill';
  SQL
end
